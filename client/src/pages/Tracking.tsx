  ...
  return (
    <div className="pb-24 pt-8 px-4 max-w-md mx-auto min-h-screen bg-background">
      ...
      {activeOrders.length > 0 && (
        <div className="mb-8">
          <h2 className="text-lg font-semibold mb-3 text-primary">{t("tracking.activeOrders")}</h2>
      ...
      {activeOrders.length === 0 && (
        <div className="text-center py-8 mb-8">
          <p className="text-muted-foreground">{t("tracking.noActiveOrders")}</p>
        </div>
      )}
      ...
      {completedOrders.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold mb-3">{t("tracking.orderHistory")}</h2>
          <div className="space-y-3">
            {completedOrders.map((order, index) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.1 }}
                className="bg-card rounded-xl p-3 shadow-sm border border-border/50 hover:shadow-md transition-all"
                data-testid={`card-order-history-${order.id}`}
              >
      ...
      {completedOrders.length === 0 && activeOrders.length > 0 && (
        <div className="text-center py-8">
          <p className="text-muted-foreground text-sm">{t("tracking.noCompletedOrders")}</p>
        </div>
      )}
    </div>
  );
